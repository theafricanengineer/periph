...
// Limitation is 64kb per stream, then daisy chain streams up to ~250k/s if
// USB is being used.
//
// GPL; so not to be used:
// https://github.com/626Pilot/RaspberryPi-NeoPixel-WS2812/blob/master/ws2812-RPi.c
//
// UNLICENSE:
...
func (d dmaStatus) GoString() string {
	var out []string
	...
	if d != 0 {
		out = append(out, fmt.Sprintf("dmaStatus(0x%08x)", d))
	}
	if len(out) == 0 {
		return "0"
	}
...
func (d dmaTransferInfo) GoString() string {
	var out []string
	...
	d &^= dmaPerMapMask
	if d != 0 {
		out = append(out, fmt.Sprintf("dmaTransferInfo(0x%08x)", d))
	}
	if len(out) == 0 {
		return "0"
	}
...
func (d dmaDebug) GoString() string {
	var out []string
	...
	if v := d & dmaVersionMask; v != 0 {
		out = append(out, fmt.Sprintf("v%d", v>>dmaVersionShift))
		d &^= dmaVersionMask
	}
	if v := d & dmaStateMask; v != 0 {
		out = append(out, fmt.Sprintf("state(%x)", v>>dmaStateShift))
		d &^= dmaStateMask
	}
	if v := d & dmaIDMask; v != 0 {
		out = append(out, fmt.Sprintf("#%x", v>>dmaIDShift))
		d &^= dmaIDMask
	}
	if v := d & dmaOutstandingWritesMask; v != 0 {
		out = append(out, fmt.Sprintf("OutstandingWrites=%d", v>>dmaOutstandingWritesShift))
		d &^= dmaOutstandingWritesMask
	}
	if d != 0 {
		out = append(out, fmt.Sprintf("dmaDebug(0x%x)", d))
	}
	if len(out) == 0 {
		return "0"
	}
...
func (d dmaStride) GoString() string {
	y := (d >> 16) & 0xFFFF
	if y != 0 {
		return fmt.Sprintf("0x%x,0x%x", y, d&0xFFFF)
	}
	return fmt.Sprintf("0x%x", d&0xFFFF)
}
...
func (c *controlBlock) GoString() string {
	return fmt.Sprintf("{\n  transferInfo: %#v,\n  srcAddr:      0x%x,\n  dstAddr:      0x%x,\n  txLen:        %v,\n  stride:       %#v,\n  nextCB:       0x%x,\n  reserved:     %v,\n}", c.transferInfo, c.srcAddr, c.dstAddr, c.txLen, c.stride, c.nextCB, c.reserved)
}
...
func uint32ToBit(w gpio.Bits, d []uint8, bit uint8, skip int) {
	// Little endian.
	x := bit / 8
	d = d[x:]
	bit -= 8 * x
...
// readStream streams input from a pin.
func readStream(p *Pin, resolution time.Duration, r gpio.Bits) error {
...
// writeStream streams data to a pin.
func writeStream(p *Pin, w gpio.Stream) error {
	...
	if err := stream.Raster32(w, resolution, bufClear.Uint32(), bufSet.Uint32(), mask, mask); err != nil {
		return err
	}
...
func smokeTest() error {
	...
	return pmem.CopyTest(size, holeSize, alloc, f)
}
